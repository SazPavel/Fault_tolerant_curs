...
void deck_create(struct card *deck, int n)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        switch(deck[i].rank) {
            case R_ACE: deck[i].value = 11; break;
            case R_KING: deck[i].value = 4; break;
            case R_QUEEN: deck[i].value = 3; break;
            case R_JACK: deck[i].value = 2; break;
            default: deck[i].value = 14 - deck[i].rank;
        }
    }
}
